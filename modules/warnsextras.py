...
class WarnsExtras(commands.Cog):
    ...
    async def allwarns_slash(self, interaction: discord.Interaction):
        if not is_moderator(interaction.user, min_level=1): return await interaction.response.send_message("Moderator Level 1 required.", ephemeral=True)
        await self._do_allwarns(interaction)

    @commands.command(name="allwarns")
    async def allwarns_prefix(self, ctx):
        if not is_moderator(ctx.author, min_level=1): return await ctx.reply("Moderator Level 1 required.")
    ...
    async def clearwarns_slash(self, interaction: discord.Interaction, member: discord.Member):
        if not is_moderator(interaction.user, min_level=2): return await interaction.response.send_message("Moderator Level 2 required.", ephemeral=True)
        await self._do_clearwarns(interaction, member)

    @commands.command(name="clearwarns")
    async def clearwarns_prefix(self, ctx, member: discord.Member):
        if not is_moderator(ctx.author, min_level=2): return await ctx.reply("Moderator Level 2 required.")
    ...
    async def resetwarns_slash(self, interaction: discord.Interaction):
        if not is_moderator(interaction.user, min_level=3): return await interaction.response.send_message("Moderator Level 3 required.", ephemeral=True)
        await self._do_resetwarns(interaction)

    @commands.command(name="resetwarns")
    async def resetwarns_prefix(self, ctx):
        if not is_moderator(ctx.author, min_level=3): return await ctx.reply("Moderator Level 3 required.")
